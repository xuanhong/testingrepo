...
class XenForo_UserUpgradeProcessor_PayPal
{
	...
	public function validateRequest(&$errorString)
	{
		...
		if (strtolower($this->_filtered['business']) != trim(strtolower(XenForo_Application::get('options')->payPalPrimaryAccount))
			&& strtolower($this->_filtered['receiver_email']) != trim(strtolower(XenForo_Application::get('options')->payPalPrimaryAccount))
		)
		{
