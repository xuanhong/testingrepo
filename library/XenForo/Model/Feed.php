...
class XenForo_Model_Feed extends XenForo_Model
{
	...
	{
		$whereConditions = $this->prepareFeedConditions($conditions, $fetchOptions);
	 ...
	 * @return string
	 */
	public function prepareFeedConditions(array $conditions, array &$fetchOptions)
	{
		$sqlConditions = array();
	 ...
	 * Prepares and imports the data from all feeds due to be imported
	 *
	 * @param $maxRunTime integer Maximum number of seconds this can run for. Note that this can potentially be over run by a slow requst.
	 */
	public function scheduledImportTimeLimited($maxRunTime)
	{
		$s = microtime(true);
	...
	/**
	 * Fetch feeds for ACP quick search based on title match
	 *
	 * @param string $searchText
	 *
	 * @return array
	 */
	public function getFeedsForAdminQuickSearch($searchText)
	{
