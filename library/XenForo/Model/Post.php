...
class XenForo_Model_Post extends XenForo_Model
{
	...
	public function movePostsToNewThread(array $posts, array $sourceThreads, array $newThread, array $options = array())
	{
		...
		$forum = $this->getModelFromCache('XenForo_Model_Forum')->getForumById($newThread['node_id']);
		if (!$forum)
		{
			return false;
		}
		...
		$newThreadUrl = XenForo_Link::buildPublicLink('threads', $newThread);
			...
			if ($threadValid)
			{
				...
				if ($options['log'])
				{
					XenForo_Model_Log::logModeratorAction(
						'thread', $sourceThread, 'post_move_source', array('url' => $newThreadUrl, 'title' => $newThread['title'])
					);
				}
			}
			else
		...
		XenForo_Application::defer('SearchIndexPartial', array('contentType' => 'post', 'contentIds' => $reindexPostIds));

		if ($options['log'])
		{
			XenForo_Model_Log::logModeratorAction(
				'thread', $newThread, 'post_move_target', array('ids' => implode(', ', $postIds))
			);
		}
