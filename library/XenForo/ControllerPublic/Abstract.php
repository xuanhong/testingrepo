...
abstract class XenForo_ControllerPublic_Abstract extends XenForo_Controller
{
	...
	protected function _assertIpNotBanned()
	{
		...
		if (XenForo_Application::isRegistered('session'))
		{
			...
			if ($session->isRegistered('isIpBanned'))
			{
				$result = $session->get('isIpBanned');
			}
		}
		else
		{
		...
		if ($result === null)
		{
			...
			$result = ($bannedIps && $this->ipMatch($this->_getClientIps(), $bannedIps));

			if ($session)
			{
				$session->set('isIpBanned', $result);
			}
		}
	...
	*/
	...
	protected function _isDiscouraged()
	{
		...
		{
			$session = XenForo_Application::getSession();
			if ($session->isRegistered('isDiscouraged'))
			{
				return $session->get('isDiscouraged');
			}
		}
		else
		{
		...
		if ($session)
		{
			$session->set('isDiscouraged', $result);
		}
