...
class XenForo_Importer_XenForo extends XenForo_Importer_Abstract
{
	...
	public function stepAttachments($start, array $options)
	{
		...
		{
			$groupedAttachments[$attachment['content_type']][$attachment['content_id']] = $attachment;
		}
		...
		foreach ($groupedAttachments AS $contentType => $attachments)
		{
			...
			$contentIdMap = $model->getImportContentMap($contentType, array_keys($attachments));
			...
			foreach ($attachments AS $contentId => $attachment)
			{
