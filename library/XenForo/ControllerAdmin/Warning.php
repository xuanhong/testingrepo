...
class XenForo_ControllerAdmin_Warning extends XenForo_ControllerAdmin_Abstract
{
	...
	public function actionActionSave()
	{
		...
		$dwInput = $this->_input->filter(array(
			...
			'ban_length_type' => XenForo_Input::STRING,
			'ban_length' => XenForo_Input::UINT,
			'extra_user_group_ids' => array(XenForo_Input::UINT, 'array' => true)
		));

		if ($dwInput['action'] == 'ban_length')
		{
			if ($this->_input->filterSingle('ban_length_type_base', XenForo_Input::STRING) == 'permanent')
			{
				$dwInput['ban_length_type'] = 'permanent';
			}
		}
		else
		{
