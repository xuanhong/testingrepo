...
!function($, window, document, _undefined)
{
	...
	$.extend(XenForo,
	{
		...
		plusoneButtonInit: function()
		{
			if ($(document).find('div.g-plusone').length)
			{
				...
				XenForo.loadJs('https://apis.google.com/js/plusone.js');
			}
		},
	...
	XenForo.ToggleTrigger.prototype =
	{
		...
		toggle: function(e)
		{
			...
			if (this.$trigger.data('toggle-if-pointer') && XenForo.isPositive(this.$trigger.data('toggle-if-pointer')))
			{
				if (this.$trigger.css('cursor') !== 'pointer')
				{
					return;
				}
			}
			...
			if (this.$target)
			{
				...
				var toggleClass = this.$target.data('toggle-class');
				if (this.targetVisible)
				{
					if (toggleClass)
					{
						this.$target.removeClass(toggleClass);
					}
					else
					{
						this.$target.xfFadeUp();
					}
				}
				else
				{
					if (toggleClass)
					{
						this.$target.addClass(toggleClass);
					}
					else
					{
						this.$target.xfFadeDown();
					}
				}
	...
	XenForo.CheckAll = function($control)
	{
		if ($control.is(':checkbox'))
		{
			...
			{
				getCheckBoxes().prop('checked', e.target.checked);
			});
		}
		else
		{
