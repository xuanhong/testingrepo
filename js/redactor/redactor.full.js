...
(function($){
	...
	Redactor.prototype = {
		...
		analyzeSelection: function()
		{
			...
			{
				var addSelected = function(el, stopOn, navKey)
				{
					do
					{
						while (el[navKey])
						{
							selectedEls.push(getMaxSelectable(el[navKey]));
							el = el[navKey];
						}
				...
				if (commonOriginDepth)
				{
					addSelected(origin[0], originCheck, 'nextSibling');
				}

				if (commonFocusDepth)
				{
					addSelected(focus[0], focusCheck, 'previousSibling');
				}
			}
			...
			if (origin[0] == focus[0])
			{
				if (origin[0].nodeType == 3)
				{
					if (origin[1] == 0 && focus[1] >= origin[0].nodeValue.length)
					{
						...
						selStart = selEnd = getMaxSelectable(origin[0].parentNode);
			...
			else
			{
				...
				{
					// whole thing is actually selected
					selStart = getMaxSelectable(origin[0]);
					selectedEls.push(selStart);
				}
				else
				...
				else if (focus[0].childNodes.length == focus[1])
				{
					// whole thing is actually selected
					selEnd = getMaxSelectable(focus[0]);
					selectedEls.push(selEnd);
			...
			selInfo.bookmarkSelection = function()
			{
				...
				$selStartIns = $('<ins class="start"></ins>').insertBefore(selStart);
				$selEndIns = $('<ins class="end"></ins>').insertAfter(selEnd);
